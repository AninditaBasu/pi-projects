...
You're losing about 9–10 seconds between each video.
...
Even though you're capturing 1-minute videos, the next one doesn't start immediately at the end of the previous one. Rather, there's a small delay while one `libcamera-vid` command finishes and the next one starts. To not lose any frames, use a background recording, by tweaking the bash script a bit.
...
This change slightly reduces how long it takes for the next loop to begin. It is by no means seamless, but it at least reduce the 10s delay to about 2–3s.